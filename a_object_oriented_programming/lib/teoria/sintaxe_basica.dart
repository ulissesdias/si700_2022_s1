/*
  Dart permite que se importe uma biblioteca com o comando "import". Note a seguir a importação da biblioteca matemática para usar a classe Random para geração de números aleatórios.
*/
...
/*
   Você também tem a opção de deixar a inferência automática definir
   os tipos por você. Nesse caso, as variáveis terão tipos em tempo de compilação, 
   mas você não precisa dizer explicitamente quais são.
*/
...
/*
   Use is para verificar se uma variável é de um dado tipo e 
   use runtimeType para receber o tipo. A seguir mostramos o uso de ambos os comandos.
*/
String obterTipo() {
  var altura = 1.74;
...
/*
    Use a palavra reservada dynamic se desejar mudar o 
    tipo de uma variável em tempo de execução. Nesse caso, a variável poderá
    mudar de tipo sempre que necessário. Note a seguir a variável altura mudando
    de double para string e posteriormente para boolean.
*/
String tipoDinamico() {
  dynamic altura = 1.74;
...
/*
  Existem duas formas de definir constantes em Dart, usando a palavra reservada const e a palavra reservada final. A diferença entre ambos é que const exige a definição do valor em tempo de compilação, enquanto que final permite a definição em tempo de execução.
*/
...
/*
  Em Dart podemos converter valores antes de atribuir para uma variável. A conversão deve ser, em geral, informada explicitamente. As conversões para string usam toString, as conversões para tipos numéricos usam parse.
*/
...
/*
  Dart disponibiliza uma série de estruturas de dados de maneira pré-definida: sequências, dicionários e conjuntos. A seguir exemplificamos o principal uso de todas elas.
*/
...
/*
 ...
 Map: Pares "chave : valor". Use como uma maneira eficiente de indexar um valor por outro valor qualquer. 
 */
...
/*
  No caso a seguir, fazemos um dicionário e já
  forçamos o tipo da key e do valor
*/
...
/*
  Os operadores aritméticos em Dart não possuem nenhum segredo e seguem o padrão da maioria das linguagens de programação. Veja a seguir vários exemplos.
*/
...
/*
  Em Dart, é possível definir um novo tipo usando a palavra reservada enum. Nesse caso, o novo tipo permite apenas um conjunto discreto (enumerável) de valores.
*/
...
String operadoresCondicionais() {
  ...
  /*
    O comando switch permite decidir entre diversas opções. Você provavelmente irá usar de maneira pareada com o comando enum.
  */
  switch (disciplina) {
...
/*
 Para laços de repetição, temos os comandos "for" e "while" seguindo um padrão comum nas linguagens baseadas em C. Temos também um "for" baseado em python.
*/
...
/*
   ...
   3 - Todas as funções retornam alguma coisa. Um "return null"  está implícito se nada for dito.
...
*/
/*
  Lançando Exceções: vamos criar um método que lança exceções usando a palavra reservada "throw". Imagine este método como se fosse uma função pouco confiável que pode lançar pelo menos três tipos de exceções.
*/
lancandoExcecoes() {
...
/*
  Para capturar a exceção, usamos "catch" ou "on". O bloco onde o erro pode ocorrer deve estar no escopo do comando "try".
*/
capturandoExcecoes() {
  try {
    lancandoExcecoes();
